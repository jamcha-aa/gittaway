...
「今回は『あの黒い画面』を [Travis CI](https://travis-ci.org/) にお願いしました。理由はTravisおじさんがプリングルズみたいなものすごいひげで、あともう一人いるんだけどその人の唇が寒中水泳の…」  
...
「準備ができたら `Generate token` をクリックする。トークンが発行されるから、この文字列をTravisのSettings画面にある `Environment Variables` のパスワード欄にコピペする。 変数名は `GITHUB_TOKEN` で、それが済んだら `Add` をクリック。注意するところは、発行画面にも英語で書かれてるけど **トークンは一度しか表示されない** 」『一度だけか？』「そう。タブを閉じたらGitHubの設定画面を開いても再表示されない。それくらい漏れたら危険なもの。ペーストしたらすぐクリップボードの履歴も消しましょう」
...
「それみたことか」  
『おい…YAML語とやらを書いたのは君だろ？』「そうですけど？」『開き直るのはいいから、原因は？』「これから調べるんですけど？」『…』

![travis-log](./travis-log.png)

『どうだ？わかりそうか？』「たぶん…、あ、ディレクトリの場所が違ってたんだ」『YAML語の文法間違いってところか』「そうそう。Travisおじさんには正しい言葉遣いじゃないと通じないからね」
...
『じゃあやってみるか』  
「うん。 `revert` でGitコマンドを使わなきゃいけないから、当初の目的は失敗だけどね」
